<template>
  ...
  <div class="page">
    ...
    <section class="card">
      <h3>① Props + Emits</h3>
      <p class="desc">
        Props 是父组件传递给子组件的数据（只读，子组件不应直接修改）。 Emits
        是子组件向父组件发送事件的方式。
      </p>
      <div class="demo-area">
        <p>
    ...
    <section class="card">
      <h3>② v-model 双向绑定</h3>
      <p class="desc">
        v-model 是 :modelValue + @update:modelValue 的语法糖。 Vue 3 支持多个
        v-model: v-model:title, v-model:content。 也可以用 .lazy 等修饰符。
      </p>
      <div class="demo-area">
        <p>当前值: {{ formValue }}</p>
    ...
    <section class="card">
      ...
      <div class="demo-area">
        <SlotDemo>
          ...
          <template #header>
            <h4 style="color: #42b883">这是具名插槽 #header 的内容</h4>
          </template>
