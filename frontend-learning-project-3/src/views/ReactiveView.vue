  ...
  <div class="page">
    ...
    <section class="card">
      ...
      <p class="desc">
        ...
        可以获取新值和旧值，支持 deep（深度监听）和 immediate（立即执行）选项。
      </p>
      <div class="demo-area">
        <label>
          搜索关键词:
          <input v-model="searchKeyword" placeholder="输入搜索词..." />
        </label>
    ...
    <section class="card">
      ...
      <p class="desc">
        watchEffect 会立即执行一次，并在执行过程中自动追踪所有响应式依赖。
        依赖变化时自动重新执行。不需要手动指定监听目标。
      </p>
      <div class="demo-area">
        <label>
          宽度:
          <input v-model.number="width" type="range" min="50" max="300" />
          {{ width }}px
        </label>
        <label>
          高度:
          <input v-model.number="height" type="range" min="50" max="300" />
          {{ height }}px
        </label>
    ...
    <section class="card">
      ...
      <div class="demo-area">
        ...
        <button @click="state.count++">修改 reactive (会触发更新)</button>
