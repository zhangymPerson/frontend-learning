...
- **不确定时必须提问确认**：当需求、目标、范围、实现方式或任何关键信息不明确时，AI 必须先向用户提问澄清，不得直接运行任务或修改文件。
- **禁止擅自推测执行**：不要基于猜测执行任务，尤其是涉及删除、覆盖、Git 操作、安装依赖、修改配置或影响运行时的行为。
- **高风险操作需授权**：破坏性操作（如 `rm -rf`、重置数据库、强制推送、停止服务）和 outward-facing 操作（如推送代码、创建 PR/评论、发送消息）必须得到用户明确授权后方可执行。
- **优先保持工作区整洁**：只在请求涉及的文件和模块范围内修改，避免无关重构或格式化改动。
